//===- SimplexTest.cpp - Tests for PresburgerSet---------------------------===//
//
...
FlatAffineConstraints makeFACFromIneq(unsigned dims, ArrayRef<SmallVector<int64_t, 4>> ineqs) {
  return makeFACFromConstraints(dims, ineqs, {});
}

PresburgerSet makeSetFromFACs(unsigned dims, ArrayRef<FlatAffineConstraints> facs) {
...
TEST(SetTest, containsPoint) {
  PresburgerSet set1 = makeSetFromFACs(1, {
      makeFACFromIneq(1, {
        {1, -2},    // x >= 2.
        {-1, 8}}),  // x <= 8.
      makeFACFromIneq(1, {
  ...
  PresburgerSet set2 = makeSetFromFACs(2, {
      makeFACFromIneq(2, {
        {1, 1, -2},  // x + y >= 4.
        {-1, -1, 30},// x + y <= 32.
        {1, -1, 0},  // x - y >= 2.
...
TEST(SetTest, Subtract) {
  ...
  {{1, 2},  {2, 2},  {4, 2},   {5, 2},  {7, 2},  {8, 2},
   {11, 2}, {1, 1},  {2, 1},   {4, 1},  {5, 1},  {7, 1},
   {8, 1},  {11, 1}, {1, 10},  {2, 10}, {4, 10}, {5, 10},
   {7, 10}, {8, 10}, {11, 10}, {1, 11}, {2, 11}, {4, 11},
   {5, 11}, {7, 11}, {8, 11},  {11, 11}});
  ...
  {{1, 1}, {2, 2}, {10, 10}, {11, 11}, {5, 4}, {7, 4}, {5, 8}, {7, 8}, {4, 4},
   {8, 4}, {4, 8}, {8, 8}});
  ...
  {{0, 0}, {1, -1}, {2, -2}, {-1, 1}, {-2, 2}, {1, 1}, {-1, -1}, {-1, 1},
   {1, -1}});
  ...
  {{1, 2}, {2, 2}, {3, 2}, {4, 2}, {1, 1}, {2, 1}, {3, 1}, {4, 1}, {2, 0},
   {3, 0}, {4, 0}, {5, 0}, {10, 2}, {11, 2}, {10, 1}, {10, 10}, {10, 11},
   {10, 9}, {11, 10}, {10, -6}, {11, -6}, {24, 8}, {24, 7}, {17, 15},
   {16, 15}});
  ...
  // ((-infinity, -5] U [3, 3] U [4, 4] U [5, 5]) - ([-2, -10] U [3, 4] U [6, 7])
  testSubtractAtPoints(
      makeSetFromFACs(1, {
        makeFACFromIneq(1, {
          {-1, -5}, // x <= -5.
