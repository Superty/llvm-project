...
Optional<TPInt> IntegerRelation::computeVolume() const {
  ...
  TPInt count = 1;
  SmallVector<TPInt, 8> dim(getNumVars() + 1);
...
void IntegerRelation::addLocalFloorDiv(ArrayRef<TPInt> dividend,
  ...
  SmallVector<TPInt, 8> dividendCopy(dividend.begin(), dividend.end());
...
IntegerRelation::unionBoundingBox(const IntegerRelation &otherCst) {
  ...
  SmallVector<TPInt, 4> minLb(getNumSymbolVars() + 1);
  SmallVector<TPInt, 4> maxUb(getNumSymbolVars() + 1);
  ...
  for (unsigned d = 0, e = getNumDimVars(); d < e; ++d) {
    ...
    std::transform(newLb.begin() + getNumDimVars(), newLb.end(),
                   newLb.begin() + getNumDimVars(), std::negate<TPInt>());
