...
public:
  ...
  AffineExpr matched();
  std::optional<int> getMatchedConstantValue();
...
std::optional<int> AffineExprMatcher::getMatchedConstantValue() {
...
std::optional<SDBMExpr> SDBMExpr::tryConvertAffineExpr(AffineExpr affine) {
          ...
          if (auto varConverted = converted.dyn_cast<SDBMTermExpr>())
            ...
            return SDBMStripeExpr::get(
                ...
                SDBMConstantExpr::get(dialect,
                                      C.getMatchedConstantValue().value()));
        }
      }
